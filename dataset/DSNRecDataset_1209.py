# coding=utf8
#把评论变成可选择是否训练的，即去掉了get_comment_vec这一步 #DSNRecDataset_1203.py
#把[source][train]、[source][test]、[target][tarin]作为训练集，[target][test](user用[source][test]里的评论)作为测试集，即suser, tuser = self.tgt_user, self.src_user变成suser, tuser = self.src_user, self.src_user  #DSNRecDataset_1209.py
#新加了for u in self.overlap_user:   #这一行和下一行是新加的，说明训练的时候源域所有评论都参加训练
...
#         self.output_dir = output_dir
    ...
    def generateTrainBatch(self, type, batchSize):
        ...
        return self._generateBatch(data, batchSize, "train"),self.srctrainbatch,self.tgttrainbatch#max(self.srctrainbatch,self.tgttrainbatch)#(max(self.srctrainbatch,self.tgttrainbatch)//min(self.srctrainbatch,self.tgttrainbatch)+1) *min(self.srctrainbatch,self.tgttrainbatch)
...
#             print('self.train_num_batch:',self.train_num_batch)
        ...
        if train_or_test == "train":
            ...
            suser, tuser = self.src_user, self.src_user #self.tgt_user, self.src_user
