...
class DSNRecDataset:
    ...
    def __getUserTrainTest(self):
        ...
        #####ranking
        def map_rat(rat):
            if rat>self.neg_start:
                return [1,0]
            else:
                return [0,1]
        df_SrcTrain['rating'] = map(map_rat,df_SrcTrain['rating'].tolist())
        df_SrcTest['rating'] = map(map_rat,df_SrcTest['rating'].tolist())
        df_TgtTrain['rating'] = map(map_rat,df_TgtTrain['rating'].tolist())
        df_TgtTest['rating'] = map(map_rat,df_TgtTest['rating'].tolist())
